...
import { createInvoiceColumns } from "../[id]/managements-list/components/columns-invoices";
import { TrackDetailModal } from "../[id]/managements-list/components/track-detail-modal";
import { updateTableProfile } from "../[id]/managements-list/services";
...
interface ManagementHistoryContentProps {
  debtorId: string;
  showBreadcrumb?: boolean;
  title?: string;
}

export const ManagementHistoryContent = ({
  debtorId,
  showBreadcrumb = false,
  title = "Lista de gestiones",
}: ManagementHistoryContentProps) => {
  ...
  const fetchDebtor = async () => {
    ...
    try {
      const debtor = await getDebtorById(session.token, profile.client.id, debtorId);
    ...
    return invoices.map((invoice) => {
      ...
      if (matchingContact) {
        return {
          ...
          track: {
            ...invoice.track,
            contact: { ...invoice.track.contact, name: matchingContact.name },
          },
        };
      }
      return invoice;
  ...
  const fetchTracks = async (currentPage: number, currentLimit: number, search?: string) => {
    if (!session?.token || !profile?.client?.id || !debtorId) return;
    setLoading(true);
    try {
  ...
  const handleUpdateColumns = async (config?: Array<{ name: string; is_visible: boolean }>) => {
    try {
  ...
  const enrichedAndSortedInvoices = useMemo(() => {
    const enriched = enrichContactWithName(invoicesWithTracks, dataDebtor?.contacts || []);
            ...
            <BreadcrumbList>
              <BreadcrumbItem>
                <BreadcrumbLink asChild>
                  <Link href="/dashboard/debtor-management" className="text-blue-600">
                    Gestión de deudores
                  </Link>
                </BreadcrumbLink>
              </BreadcrumbItem>
              ...
              <BreadcrumbItem>
                <BreadcrumbLink asChild>
                  <Link href={`/dashboard/debtor-management/${dataDebtor?.id}`} className="text-blue-600">
      ...
      <Card>
        <CardContent className="space-y-3">
          ...
          <div className="bg-blue-100/30 p-4 rounded-lg">
            <h3 className="text-sm font-medium mb-3 text-blue-800">Datos del deudor</h3>
            {dataDebtor?.id ? (
              ...
              <p className="text-sm text-gray-600">Cargando datos del deudor...</p>
            )}
          </div>
          <DataTableDynamicColumns
            ...
            rowClassName={(invoice) => invoice.type === "CREDIT_NOTE" ? "bg-red-50" : ""}
            ctaNode={
